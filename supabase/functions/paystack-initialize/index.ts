...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!res.ok || !data.status) {
      throw Object.assign(new Error(data.message ?? 'Paystack initialization failed'), {
...
async function prepareMarketplace(supabase: any, userId: string, kind: MarketplaceKind, body: any) {
  if (kind === 'meal_order') {
    const { data: vendor, error } = await supabase
      .from('vendors')
      .select('id, delivery_fee_naira, min_order_naira, published, is_open')
      .eq('id', body.listingId)
      .eq('published', true)
      .single()
    if (error || !vendor) throw new Error('Vendor not found')
    if (vendor.is_open === false) throw Object.assign(new Error('This vendor is currently closed'), { status: 409 })

    const requested: Array<{ menu_item_id?: string; qty?: number }> = body.items ?? []
    if (!requested.length) throw new Error('Select at least one meal')
    if (requested.some((item) => !item.menu_item_id)) {
      throw Object.assign(new Error('Every meal must reference a menu item'), { status: 400 })
    }

    // Aggregate duplicate item IDs before validating stock and calculating price.
    const quantities = new Map<string, number>()
    for (const item of requested) {
      const id = item.menu_item_id!
      const qty = Math.floor(Number(item.qty) || 1)
      if (qty < 1 || qty > 100) {
        throw Object.assign(new Error('Meal quantity must be between 1 and 100'), { status: 400 })
      }
    ...
    const { data: menu, error: menuError } = await supabase
      ...
      .select('id, name, price_naira, available, quantity')
      .eq('vendor_id', vendor.id)
      .in('id', ids)
    if (menuError) throw menuError

    // Every item and price comes from the selected vendor's live menu.
  ...
  if (kind === 'trainer_booking') {
    ...
    // trainer_availability.weekday/start_min/end_min are Lagos-local wall-clock values
    // (entered via an <input type="time"> in the UI), so the check must run in that
    // timezone rather than the edge function's server timezone (UTC).
    ...
    if (slots && slots.length) {
      ...
      const ok = slots.some((s: any) => s.weekday === weekday && mins >= s.start_min && mins < s.end_min)
...
async function stampReference(supabase: any, kind: MarketplaceKind, recordId: string, reference: string, userId: string) {
  let error: any = null
  if (kind === 'meal_order') {
    ;({ error } = await supabase.from('orders').update({ paystack_reference: reference }).eq('id', recordId).eq('user_id', userId).select('id').single())
  } else if (kind === 'trainer_booking') {
    ;({ error } = await supabase.from('bookings').update({ paystack_reference: reference }).eq('id', recordId).eq('user_id', userId).select('id').single())
  } else if (kind === 'gym_membership') {
    ;({ error } = await supabase.from('gym_memberships').update({ paystack_reference: reference }).eq('id', recordId).eq('user_id', userId).select('id').single())
  } else if (kind === 'partnership') {
    ;({ error } = await supabase.from('influencer_partnerships').update({ paystack_reference: reference }).eq('id', recordId).eq('brand_user_id', userId).select('id').single())
  }
