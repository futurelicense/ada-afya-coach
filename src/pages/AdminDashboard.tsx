...
function Inquiries() {
  ...
  if (rows.length === 0) return <EmptyState icon={Inbox} title="No inquiries" description="Requests members send to businesses will show up here." />;
  return (
      ...
      {LISTING_TABLES.map((t) => (
        <TabsContent key={t.table} value={t.table}>
          {loading ? (
            ...
            <EmptyState icon={Layers} title="Nothing here yet" description="Listings in this category will appear once someone publishes one." />
          ) : (
            <ul className="space-y-2">
              {rows.map((r) => (
