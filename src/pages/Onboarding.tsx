    ...
    <div className="min-h-screen flex flex-col bg-background relative overflow-hidden">
      ...
      <div className="flex items-center gap-2 justify-center px-6 pb-8 relative z-10">
        {STEPS.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            {i < STEPS.length - 1 && (
              <div className={cn("h-px w-8 sm:w-14 transition-all duration-500", i + 1 < step ? "bg-primary" : "bg-border")} />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex-1 flex items-start justify-center px-4 pb-10 relative z-10">
        <div className="w-full max-w-md">
          <div className="glass rounded-3xl p-7 shadow-premium">

            {/* Step header */}
            <div className="text-center mb-8">
              <div className="text-4xl mb-3">{currentStep.icon}</div>
              <h2 className="font-display font-black text-2xl text-foreground mb-1">{currentStep.label}</h2>
              ...
              <div className="space-y-5 animate-fade-in">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="bg-muted border-border text-foreground placeholder:text-muted-foreground/60 focus:border-primary/50 h-12 rounded-xl"
                  />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-sm">City / area</Label>
                  <Input
            ...
            {step === 2 && (
              <div className="space-y-5 animate-fade-in">
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-sm">Current weight (kg)</Label>
                  <Input
                    type="number"
                    placeholder="e.g. 72"
            ...
            {step === 3 && (
              <div className="space-y-5 animate-fade-in">
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-sm">Primary fitness goal</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-sm">Current fitness level</Label>
                  <div className="space-y-2">
                    {LEVEL_OPTIONS.map(l => (
                      <button
                        key={l.value}
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground/70">{l.desc}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {step === 4 && (
              <div className="space-y-5 animate-fade-in">
                <div className="space-y-2">
                  <Label className="text-muted-foreground text-sm">Diet preference</Label>
                  <div className="grid grid-cols-2 gap-2">
                    {DIET_OPTIONS.map(d => (
                      <button
                        key={d.value}
                        ...
                        className={cn(
                          ...
                          formData.dietPreference === d.value
                            ? "bg-secondary/20 border-secondary/50 text-foreground"
