...
interface FoodScanResultProps {
  ...
  onMealLogged?: () => void | Promise<void>;
}
  ...
  const handleLogMeal = async () => {
    ...
    try {
      ...
      await userDataService.addMeal({
        ...
        protein: Math.round(result.food.protein * multiplier * 10) / 10,
        carbs: Math.round(result.food.carbs * multiplier * 10) / 10,
        fats: Math.round(result.food.fats * multiplier * 10) / 10,
        eaten: true,
      });
      foodScanService.saveScanToHistory(result);
      await onMealLogged?.();
      toast({ title: "Meal logged", description: `${result.food.name} was added to today's nutrition.` });
      onOpenChange(false);
    } catch (error) {
      toast({
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto p-0">
        ...
        <DialogFooter className="p-4 pt-0 flex-col sm:flex-row gap-2">
          ...
          <div className="flex gap-2 w-full sm:flex-1">
            <Button onClick={handleLogMeal} disabled={loggingMeal} className="flex-1 gap-2">
