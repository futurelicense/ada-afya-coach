...
import type { WorkoutSession } from "@/lib/userDataService";
...
const defaultWorkoutExercises: Exercise[] = [
  {
    name: "Push-ups",
    duration: 30,
    rest: 15,
...
interface VoiceGuidedWorkoutProps {
  workout?: WorkoutSession | null;
  onComplete?: (workout: WorkoutSession) => void | Promise<void>;
}

export const VoiceGuidedWorkout = ({ workout, onComplete }: VoiceGuidedWorkoutProps) => {
  const workoutExercises: Exercise[] = useMemo(() => workout?.exercises.length
    ? workout.exercises.map(exercise => ({
        name: exercise.name,
        duration: Math.max(30, exercise.sets * Math.max(exercise.reps, 1) * 2),
        rest: 20,
        formCues: [
          `Complete ${exercise.sets} sets of ${exercise.reps} reps`,
          `Focus on ${exercise.muscles || "controlled form"}`,
          "Keep breathing and move with control",
          "You're halfway there",
          "Finish strong",
        ],
      }))
    : defaultWorkoutExercises, [workout]);
  ...
  useEffect(() => {
        ...
        if (prev <= 1) {
          ...
          if (isResting) {
            ...
            if (currentExerciseIndex < totalExercises - 1) {
              ...
              if (workout) void onComplete?.(workout);
              return 0;
            }
          } else {
    ...
    isActive, isResting, currentExerciseIndex, cueIndex, currentExercise.duration,
    currentExercise.formCues, currentExercise.rest, onComplete, speak,
    totalExercises, workout, workoutExercises,
  ]);
      ...
      <CardHeader className="p-4 sm:p-6">
        <CardTitle className="flex items-center justify-between text-base sm:text-lg">
          <span className="flex items-center gap-2">
            ...
            {workout ? `Guided: ${workout.name}` : "Voice-Guided Workout"}
          </span>
          <Button
            variant="ghost"
            size="sm"
