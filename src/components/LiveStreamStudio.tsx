...
import { loadAgoraSdk } from "@/lib/agoraLoader";
...
export function LiveStreamStudio({ trainerId, trainerName }: Props) {
  ...
  async function goLive() {
    ...
    try {
      // The server derives the channel and host entitlement from this session.
      const tokenRes = await fetch(`${FUNCTIONS_URL}/live-token`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${session.access_token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId: sess.id, role: 'host' }),
      });
      const { token, appId, channelName: authorizedChannel, uid, error: tokenErr } = await tokenRes.json();
      if (!tokenRes.ok || tokenErr || !appId) {
        throw new Error(tokenErr || "Live streaming isn't configured yet");
      }
      ...
      await client.join(appId, authorizedChannel, token, uid);
      ...
      if (videoRef.current) camera.play(videoRef.current);
      await client.publish([audio, camera]);

      setCameraOn(true);
      setSessionId(sess.id);
      setChannelName(authorizedChannel);
      setIsLive(true);
      setStartedAt(new Date());
      setChat([]);
      toast({ title: "You're live", description: "Your session is now visible to premium members." });
    } catch (error) {
      await supabase.from('live_sessions').delete().eq('id', sess.id);
      await clientRef.current?.leave();
      clientRef.current = null;
      toast({
        title: "Could not start live video",
        ...
        <Card className="glass border-0 shadow-premium overflow-hidden">
          <div className="relative aspect-video bg-gradient-to-br from-primary/30 to-secondary/30">
            ...
            <div className="absolute top-4 left-4 flex items-center gap-2">
              <Badge className="bg-destructive text-white border-0 gap-1.5 px-3 py-1.5 text-sm animate-pulse">
      ...
      <Card className="glass border-0 shadow-card flex flex-col h-[520px]">
        <CardHeader className="pb-3 border-b border-border/50 flex-shrink-0">
          <CardTitle className="text-base flex items-center gap-2">
            Live Chat
            <Badge className="bg-destructive/10 text-destructive border-destructive/20 text-xs">{chat.length} messages</Badge>
          </CardTitle>
        </CardHeader>
