...
const AUTOMATIC_GOAL_TYPES = new Set(["workouts", "calories", "streak", "weight"]);
...
export const GoalSetting = () => {
  ...
  const addGoal = async () => {
    if (!newGoal.title || newGoal.target === null || newGoal.target === undefined || !newGoal.deadline) {
    ...
    if (editingId) {
      ...
      await userDataService.addGoal({ ...newGoal, startValue: newGoal.current, completed: false });
  ...
  const calculateProgress = (goal: Goal) => {
    ...
    if (goal.type === 'weight' && goal.startValue > goal.target) {
      const totalToLose = goal.startValue - goal.target;
      const lost = goal.startValue - goal.current;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {goals.map((goal) => {
          ...
          return (
            <Card key={goal.id} className={`hover-scale shadow-card ${isCompleted ? 'border-success' : ''}`}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {AUTOMATIC_GOAL_TYPES.has(goal.type) && <Badge variant="outline">Auto</Badge>}
                    </CardTitle>
              ...
              <CardContent className="space-y-4">
                ...
                {!isCompleted && !AUTOMATIC_GOAL_TYPES.has(goal.type) && (
                ...
                {!isCompleted && AUTOMATIC_GOAL_TYPES.has(goal.type) && (
                  <p className="text-xs text-muted-foreground">
                    Progress updates automatically from your profile and completed activity.
                  </p>
                )}
      ...
      {goals.length === 0 && !showForm && (
        <Card>
          <EmptyState
            ...
            description="Set a target so you can track your progress toward it."
