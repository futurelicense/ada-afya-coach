...
function goalFromRow(row: any): Goal {
  return {
    ...
    startValue: row.start_value !== null && row.start_value !== undefined ? Number(row.start_value) : Number(row.current),
    unit: row.unit,
...
function goalToRow(goal: Partial<Goal>) {
  ...
  if (goal.startValue !== undefined) row.start_value = goal.startValue;
...
class UserDataService {
  ...
  async addMeal(meal: Omit<MealLog, 'id'> & { id?: string }): Promise<void> {
    const userId = await currentUserId();
    if (!userId) throw new Error('Sign in to log a meal');
    const { error } = await supabase.from('meal_logs').insert({ user_id: userId, ...mealToRow(meal) });
    if (error) throw error;
  }
  ...
  async getTotalStats() {
    const userId = await currentUserId();
    if (!userId) {
      return { totalWorkouts: 0, totalCaloriesBurned: 0, totalActiveMinutes: 0, totalMealsLogged: 0, goalsAchieved: 0, currentStreak: 0 };
    }

    const [{ data: workouts }, { data: meals }, { data: goals }, currentStreak] = await Promise.all([
      supabase.from('workout_sessions').select('calories_burned, duration').eq('user_id', userId).eq('completed', true),
    ...
    return {
      ...
      totalActiveMinutes: (workouts ?? []).reduce((sum, w: any) => sum + (w.duration ?? 0), 0),
