...
export const foodScanService = {
  ...
  findAlternatives(currentFood: FoodItem, profile: UserHealthProfile): FoodItem[] {
    return nigerianFoodDatabase
      .filter(food => {
        ...
        if (food.allergens.some(a => profile.allergies.some(pa => pa.toLowerCase().includes(a.toLowerCase())))) return false;
        // Same category
  ...
  // Identify food from a photo using Claude Vision (via Supabase Edge Function)
  async recognizeMealPhoto(imageData: string, mediaType = 'image/jpeg'): Promise<{ foods: FoodItem[], confidence: 'high' | 'medium' | 'low' }> {
    const FUNCTIONS_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1`

    // Strip the data-URL prefix — Edge Function expects raw base64
    const base64 = imageData.replace(/^data:[^;]+;base64,/, '')

    try {
      const { data: { session } } = await supabase.auth.getSession()

      const res = await fetch(`${FUNCTIONS_URL}/scan-food`, {
        method:  'POST',
        headers: {
      ...
      // Map Edge Function response to FoodItem[] shape
      const foods: FoodItem[] = (result.foods ?? []).map((f: any) => ({
        ...
        commonPreparations: f.commonPreparations ?? [],
        healthFlags:        f.healthFlags  ?? [],
        portionSize:        f.portionSize  ?? '1 serving',
      }))

      return { foods, confidence: result.confidence ?? 'medium' }
    } catch {
      // Fallback to local database search on error (graceful degradation)
      const shuffled = [...nigerianFoodDatabase].filter(f => f.category === 'meal').sort(() => Math.random() - 0.5)
      return { foods: shuffled.slice(0, 3), confidence: 'low' }
    }
  },
